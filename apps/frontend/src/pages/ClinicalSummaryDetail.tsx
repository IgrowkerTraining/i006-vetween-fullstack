  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        setPatientName(patientData?.nombre ?? patientData?.nombre_paciente ?? "-");
      } catch (err: any) {
      ...
      <MainLayout>
        <div className="flex flex-1 items-center justify-center py-24">
          <p className="text-sm text-red-600">
            {fetchError ?? "No se encontró el resumen clínico."}
          </p>
        </div>
      </MainLayout>
    );
        ...
        <section className="flex-1 px-8 py-6">
          ...
          <div className="rounded-b-xl rounded-tr-xl border border-border bg-card p-6 shadow-sm">
            ...
            {e.descripcion_clinica && (
              <div className="mb-4">
                <p className="mb-1 text-sm font-semibold text-foreground">Descripción clínica</p>
                <p className="text-sm leading-relaxed text-foreground">{e.descripcion_clinica}</p>
              </div>
            )}
            ...
            {e.tratamiento_indicado && (
              <div className="mb-4">
                <p className="mb-1 text-sm font-semibold text-foreground">Tratamiento indicado</p>
                <p className="text-sm leading-relaxed text-foreground">{e.tratamiento_indicado}</p>
              </div>
            )}
            ...
            {e.sintesis_visitas?.length > 0 && (
              <div className="mb-4">
                <p className="mb-2 text-sm font-semibold text-foreground">Síntesis de visitas</p>
                <div className="space-y-2">
                  {e.sintesis_visitas.map((v, i) => (
                    <div key={i} className="rounded-lg border border-border bg-muted/40 px-4 py-3 text-sm">
                      <p className="font-medium text-foreground">{v.fecha} — {v.motivo}</p>
                      {v.diagnostico && <p className="mt-0.5 text-muted-foreground">Diagnóstico: {v.diagnostico}</p>}
                      {v.tratamiento && <p className="text-muted-foreground">Tratamiento: {v.tratamiento}</p>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {e.historial_vacunas?.length > 0 && (
              <div className="mb-4">
                <p className="mb-2 text-sm font-semibold text-foreground">Historial de vacunas</p>
                <div className="space-y-2">
                  {e.historial_vacunas.map((v, i) => (
                    <div key={i} className="rounded-lg border border-border bg-muted/40 px-4 py-3 text-sm">
                      <p className="font-medium text-foreground">{v.nombre}</p>
                      <p className="text-muted-foreground">{v.fecha_aplicacion} — Estado: {v.estado}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {e.factores_riesgo?.length > 0 && (
              <div className="mb-4">
                <p className="mb-2 text-sm font-semibold text-foreground">Factores de riesgo</p>
